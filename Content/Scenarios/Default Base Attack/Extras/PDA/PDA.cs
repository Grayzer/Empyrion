...
public class MyBaseAttack : PdaScript {
    ...
    public MyBaseAttack() {
        Data.Creator = "Eleon Game Studios";
        Data.CompletedMessage = "15|As the sun sets on the horizon, blood glistens in the fading sunlight, you look at your dog brothers... without a word, it is all too well understood: this is just the beginning...";
        Data.FailedMessage = "15;t|Amid the groans and screams you look at the sky and take your final breaths, a Zirax soldier walks up to you and puts his blaster to your face...";
    }
...
public class ChapterZirax : Chapter {
    ...
    public ChapterZirax(string _id) : base(_id) {
        chapterData.ChapterTitle = "Zirax Invasion";
        chapterData.Category = ChapterCategory.UCHMission;
        chapterData.Description = "[ffff00]Prepare yourselves for Zirax invasion![-]\n\nYou are in an isolated outpost on a distant world. You have reports " +
                                  "of an imminent Zirax invasion. You must defend the outpost at all costs.\n\n[ffff00]Take this time to familiarize yourself with this base[-].\n\n" +
                                  "[ff0000]You are not tasked with mining or crafting any of your equipment. Supplies, weapons, ammo and more will be dropped on the landing pad.[-]";
...
public class InitialPrep : Task {
    ...
    public InitialPrep(string _id, System.Action<Task> _completeCallback) : base(_id) {
        ...
        data.TaskTitle = "Welcome and Prep";
        data.StartMessage = "15|[ffff00]Map markers to important containers have been provided.[-]\n\nPlease note these locations and use the signs at all times.";
        data.StartDelay = 1;
        state.Id = PdaManagerBridge.CreateId(_id);

        // wait for players to join
        ActionData startData = new ActionData();
        startData.ActionTitle = "Waiting for players: ";
        startData.Description = "Prepare for incoming Zirax invasion.";
...
public class WavePrepTask : Task {
    ...
    public WavePrepTask(string _id, System.Action<Task> _completeCallback) : base(_id) {
        ...
        // set up task data
        data.TaskTitle = "Attack Preparations";
        data.StartMessage = string.Format("10;t|Strike Team, prepare for attack! ETA in T-{0}.[-]\n\nZIRAX ATTACK IS IMMINENT THIS IS NOT A DRILL!\n\n" +
                                          "[b]I REPEAT: THIS IS [i]NOT[/i] A DRILL![/b]", prepTime);
        data.StartDelay = 3;
        ...
        setTimerActionData.ActionTitle = "Incoming ETA: ";
        setTimerActionData.Description = "Place defenses and prepare for incoming attack.";
        setTimerActionData.CompletedMessage = "7|OK ladies, that's it. This will have to do.";
...
public class WaveDefendTask : DefendTask {
    ...
    const string taskTitleFmtStr = "[b][ff0000]DEFEND WAVE: {0}[-][/b]";
    ...
    public WaveDefendTask(string _id, System.Action<Task> _onTaskComplete, int _waveNumber = 1) : base(_id) {
        ...
        data.StartMessage = "10;t|[ffff00]Strike Team, take positions![-]";
        data.CompletedMessage = "10|That's the last of them.\n\nStandby, Orbital Command is sending an [ffff00]air drop to re-supply[-].\n\nGet out there and [ff0000]bring it back![-]";
        data.StartDelay = 2;
...
public class RetakeBaseTask : Task {
    ...
    string completedMessageSuccess = "10;t|That was too close girls, if you let this happen again, you will have to answer to me!";
    string completedMessageFail = "[ffff00]The Zirax have captured your base! Mission[-] [ff0000]FAILED![-]";

    public RetakeBaseTask(string _id, int _retakeTime) : base(_id) {
        // set up task data
        data.TaskTitle = "Reclaim the Base!";
        data.StartMessage = "10;t|We must reclaim our base - otherwise it is over! Now move!";
        data.StartDelay = 1;
